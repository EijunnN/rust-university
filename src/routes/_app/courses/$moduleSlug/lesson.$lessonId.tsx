...
export const Route = createFileRoute(
  ...
  loader: async ({ params }): Promise<LoaderData> => {
    ...
    const rendered = await Promise.all(
      lesson.blocks.map(async (b: Block): Promise<RenderedBlock> => {
        switch (b.type) {
          case 'text':
...
function LessonPage() {
  ...
  const { module, lesson, blocks, prevLessonId, nextLessonId, isLastInModule } = data
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] min-h-[calc(100vh-3.5rem)]">
      {/* Desktop sidebar */}
      ...
      <main className="px-4 sm:px-6 py-8 sm:py-10 max-w-3xl mx-auto w-full">
        ...
        <nav className="text-xs text-muted-foreground mb-4 flex items-center gap-1 flex-wrap">
          ...
          <span>/</span>
          <span className="text-foreground font-medium">Lección {lesson.order}</span>
        </nav>
        ...
        <article className="space-y-6">
          {blocks.map((b, i) => {
            switch (b.type) {
              case 'text':
                return (
                  <MarkdownContent
                    key={i}
                    hast={b.hast}
                    className="prose prose-zinc dark:prose-invert max-w-none prose-headings:font-serif prose-pre:p-0 prose-pre:bg-transparent prose-pre:my-0 prose-code:before:content-none prose-code:after:content-none"
                  />
                )
              case 'code':
              ...
              case 'callout':
                return <CalloutBlock key={i} variant={b.variant} hast={b.hast} />
              case 'first-principles':
