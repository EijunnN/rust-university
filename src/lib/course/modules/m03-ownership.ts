...
const module: Module = {
  ...
  "description": "El corazon de Rust: comprende el sistema de ownership, referencias, borrowing y slices que hacen a Rust único.",
  "lessons": [
    {
      "id": "m03_l01",
      "moduleId": "m03",
      ...
      "blocks": [
        {
          ...
          "title": "Ownership nace de una pregunta simple: ¿quién limpia la memoria?",
          "problem": "Cada dato ocupa memoria. Si el dato vive para siempre, desperdicias RAM. Si lo borras mientras alguien lo usa, rompes el programa. El problema central es decidir quién es responsable de limpiar.",
          "mentalModel": "Ownership es como una llave única de una habitación. Quien tiene la llave puede usar y finalmente cerrar la habitación. Si todos tienen llaves, nadie sabe quién debe cerrar; si nadie la tiene, la habitación queda abandonada.",
          "concreteExample": "Un `String` puede crecer, así que vive en el heap. Rust asigna un owner. Cuando ese owner sale del scope, Rust llama a `drop` y libera la memoria. No hay recolector buscando después ni un humano llamando `free` manualmente.",
          "remember": "Ownership no es una regla rara de Rust: es una respuesta concreta al problema de limpiar memoria sin pausas ni errores peligrosos."
        },
        {
          "type": "challenge",
        ...
        {
          "type": "quiz",
          "question": "¿Cuándo se libera automáticamente la memoria de una variable en Rust?",
          "options": [
            {
        ...
        {
          "type": "quiz",
          "question": "¿Cuántos owners puede tener un valor en Rust al mismo tiempo?",
          "options": [
            {
      ...
      "blocks": [
        {
          ...
          "problem": "Si dos variables creen ser dueñas del mismo dato en el heap, ambas podrían intentar liberarlo. Eso causa errores graves. Rust evita que haya dos dueños activos del mismo valor.",
          "mentalModel": "Mover un valor es entregar la llave, no hacer una fotocopia de la casa. La variable anterior deja de poder usar ese valor porque ya no es responsable de él.",
          "concreteExample": "Si `nombre` contiene un `String` y haces `let otro = nombre`, Rust mueve el ownership a `otro`. Usar `nombre` después sería como intentar entrar con una llave que ya entregaste.",
          "remember": "Move protege contra dobles liberaciones y contra confusión sobre quién debe limpiar."
        },
        {
          "type": "challenge",
        ...
        {
          "type": "quiz",
          "question": "Qué pasa cuando haces `let s2 = s1;` donde s1 es un String?",
          "options": [
            {
        ...
        {
          "type": "quiz",
          "question": "¿Por qué Rust usa 'move' en lugar de copiar un String automáticamente?",
          "options": [
            {
      ...
      "blocks": [
        {
          ...
          "problem": "A veces copiar un dato es barato, como copiar un número. Otras veces implica duplicar memoria en el heap, como copiar un texto largo. Rust no quiere esconder ese costo.",
          "mentalModel": "`Copy` es como copiar un número en una nota adhesiva. `Clone` es como pedir una copia completa de un documento grande: se puede hacer, pero debe ser intencional.",
          "concreteExample": "Un `i32` se copia automáticamente porque son pocos bytes. Un `String` no se copia automáticamente porque duplicar su contenido puede reservar memoria nueva. Por eso Rust exige `clone()` si quieres una copia real.",
          "remember": "Cuando Rust te pide `clone()`, te está diciendo: “esto puede costar memoria y tiempo; confirma que lo quieres”."
        },
        {
          "type": "challenge",
        ...
        {
          "type": "quiz",
          "question": "Qué pasa cuando pasas un String a una función sin usar & ni .clone()?",
          "options": [
            {
      ...
      "blocks": [
        {
          ...
          "problem": "Muchas funciones sólo necesitan leer un dato. Si cada lectura tuviera que tomar ownership, el código sería incómodo y movería valores innecesariamente.",
          "mentalModel": "Una referencia es pedir prestado. Puedes leer el libro de alguien sin quedártelo. Si quieres escribir en él, necesitas un préstamo mutable y exclusivo.",
          "concreteExample": "Una función que imprime un nombre no necesita quedarse con el `String`. Puede recibir `&String` o `&str`, leerlo y devolverlo al owner. Así evitas mover el dato sólo para mostrarlo en pantalla.",
          "remember": "Borrowing permite compartir acceso sin perder ownership, pero Rust controla que no haya escritura peligrosa mientras otros leen."
        },
        {
          "type": "text",
        ...
        },
        {
          ...
          "intro": "Vamos a escribir una función que lee un `String` prestado (`&String`) y devuelve su longitud, sin tomar posesión. Observa, completa y hazlo solo.",
          "stages": [
            {
              "kind": "worked",
              "instructions": "**Paso 1 — observa.** `contar_letras` recibe `&String` (un préstamo). Lee con `.len()` y NO se vuelve dueña: por eso quien la llama puede seguir usando su `String` después.",
              "code": "fn contar_letras(texto: &String) -> usize {\n    texto.len()\n}"
            },
            {
              "kind": "faded",
              "instructions": "**Paso 2 — completa.** Rellena los `___` para que `longitud` tome el `String` **prestado** y devuelva su tamaño.",
              "code": "fn longitud(s: ___String) -> usize {\n    s.___()\n}"
            },
            {
              "kind": "solo",
              "instructions": "**Paso 3 — tú solo.** Escribe `longitud` desde cero: recibe `&String` y devuelve su `len()`.",
        ...
        {
          "type": "quiz",
          "question": "¿Cuántas referencias inmutables puedes tener a la vez de un mismo dato?",
          "options": [
            {
              "text": "Solo una",
              "correct": false
        ...
        {
          "type": "quiz",
          "question": "¿Por qué Rust no permite tener una referencia mutable y una inmutable al mismo tiempo?",
          "options": [
            {
      ...
      "blocks": [
        {
          ...
          "problem": "A menudo necesitas trabajar con una porción de datos: la primera palabra de un texto, una parte de una lista, un rango de bytes. Copiar todo sería lento y gastaría memoria.",
          "mentalModel": "Un slice es una ventana sobre datos existentes. No posee la casa; sólo marca desde dónde hasta dónde estás mirando.",
          "concreteExample": "Si tienes `\"hola mundo\"` y quieres `\"hola\"`, un slice puede apuntar a esa parte del texto original. No crea otro texto completo; sólo guarda referencia y longitud.",
          "remember": "Un slice depende de que el dato original siga vivo. Por eso está conectado con lifetimes y borrowing."
        },
        {
          "type": "challenge",
          "conceptId": "m03-slice-first-word",
          "title": "Antes de leer: devuelve la primera palabra",
        ...
        {
          "type": "quiz",
          "question": "¿Cuál es la ventaja de usar un slice (`&str`) en lugar de un índice (`usize`) para representar una parte de un String?",
          "options": [
            {
    ...
    },
    {
      ...
      "blocks": [
        {
          "type": "first-principles",
          "title": "Lifetimes: demostrar que una referencia no vivirá más que su dato",
          "problem": "Una referencia es peligrosa si apunta a algo que ya fue destruido. El problema no es usar referencias; el problema es usarlas después de que el dato original dejó de existir.",
          "mentalModel": "Un lifetime es una fecha de validez. Rust revisa que tu pase de entrada no dure más que el evento al que quieres entrar.",
          "concreteExample": "Si una función devuelve una referencia a una variable creada dentro de la función, esa variable muere al terminar la función. La referencia quedaría apuntando a nada. Rust lo prohíbe antes de ejecutar.",
          "remember": "Los lifetimes no hacen vivir más a los datos; sólo describen y verifican cuánto viven."
        },
        {
          "type": "challenge",
        ...
        {
          "type": "quiz",
          "question": "¿Cuándo necesitas anotar lifetimes explícitamente en una función?",
          "options": [
            {
